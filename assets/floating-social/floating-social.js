(function () {
  ...
  function syncJoinFabOffset() {
    const fab = document.querySelector('.mobile-join-fab');
    if (!fab) {
      document.body.classList.remove('has-mobile-join-fab');
      return;
    }
    const style = window.getComputedStyle(fab);
    const visible = style.display !== 'none' && style.visibility !== 'hidden' && style.opacity !== '0';
    document.body.classList.toggle('has-mobile-join-fab', visible);
  }
  ...
  function mountFloatingSocial() {
    ...
    if (document.querySelector('.mobile-join-fab')) {
      syncJoinFabOffset();
      window.addEventListener('resize', syncJoinFabOffset, { passive: true });
      window.addEventListener('orientationchange', syncJoinFabOffset);
    }
  }
