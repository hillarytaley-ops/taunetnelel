(function () {
  ...
  function getPathPrefix() {
    ...
    if (path.includes('/members/') || path.includes('/admin/')) return '../';
    return '';
  }

  function initDonateCtas() {
    const prefix = getPathPrefix();
    const donateHref = `${prefix}sponsorship.html#donate`;

    document.querySelectorAll('.site-header .header-actions').forEach((actions) => {
      if (actions.querySelector('[data-donate-cta]')) return;

      const donate = document.createElement('a');
      donate.href = donateHref;
      donate.className = 'btn btn--accent header-donate';
      donate.dataset.donateCta = 'true';
      donate.textContent = 'Donate';

      const join = actions.querySelector('a[href$="register.html"]');
      if (join) actions.insertBefore(donate, join);
      else actions.appendChild(donate);
    });

    document.querySelectorAll('.mobile-nav__panel').forEach((panel) => {
      if (panel.querySelector('[data-donate-cta]')) return;

      const donate = document.createElement('a');
      donate.href = donateHref;
      donate.className = 'btn btn--accent';
      donate.dataset.donateCta = 'true';
      donate.textContent = 'Donate';

      const join = panel.querySelector('a[href$="register.html"]');
      if (join) panel.insertBefore(donate, join);
      else panel.appendChild(donate);
    });
  }

  function initEventStrip() {
    if (document.querySelector('.event-strip')) return;
    ...
    strip.setAttribute('aria-label', 'Taunet Nelel event update');
    strip.innerHTML = `
      <div class="container">
        <div class="event-strip__info">
          <span class="event-strip__label">Community update</span>
          <span class="event-strip__title">Taunet Nelel events, tickets, gallery & sponsorship</span>
          <p class="event-strip__also">Explore upcoming programs, view photos, or support the next community gathering.</p>
        </div>
        <div class="event-strip__actions">
          <a href="${prefix}events.html" class="btn btn--outline btn--sm">View Events</a>
          <a href="${prefix}gallery.html" class="btn btn--outline btn--sm">Find Photos</a>
          <a href="${prefix}sponsorship.html#donate" class="btn btn--accent btn--sm">Donate</a>
        </div>
      </div>`;
